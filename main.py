...
async def _download_recording_local(cc_id: str, url: str) -> None:
    """Download a Telnyx recording to data/recordings/<cc_id>.mp3 and persist the local path."""
    try:
        safe_id = _re.sub(r"[^A-Za-z0-9_.-]", "_", cc_id)[:120]
        ext = ".mp3" if url.lower().split("?")[0].endswith(".mp3") else ".wav"
        dest = RECORDINGS_DIR / f"{safe_id}{ext}"
        headers: dict[str, str] = {}
        if "api.telnyx.com" in url and "?" not in url and config.TELNYX_API_KEY:
            ...
            async with ac.stream("GET", url, headers=headers, follow_redirects=True) as r:
                ...
                with open(dest, "wb") as f:
                    async for chunk in r.aiter_bytes():
                        if chunk:
                            f.write(chunk)
        rel = f"/api/recordings/{cc_id}/file"
        update_call(cc_id, recording_local_path=str(dest), recording_local_url=rel)
        if cc_id in active_calls:
            active_calls[cc_id]["recording_local_path"] = str(dest)
            active_calls[cc_id]["recording_local_url"] = rel
        logger.info("Recording downloaded locally: %s (%d bytes)", dest, dest.stat().st_size)
    except Exception:
        logger.exception("Local recording download failed for %s", cc_id)


@app.get("/api/recordings/{call_control_id}/file")
async def serve_local_recording(call_control_id: str):
    from fastapi.responses import FileResponse
    rec = get_call_by_control_id(call_control_id)
    local = (rec or {}).get("recording_local_path") or ""
    if local and Path(local).exists():
        media = "audio/mpeg" if local.lower().endswith(".mp3") else "audio/wav"
        return FileResponse(local, media_type=media, filename=Path(local).name)
    raise HTTPException(status_code=404, detail="Local recording not found")
    ...
    try:
        ...
        # ── RECORDING SAVED ────────────────────────────
            ...
            if url:
                ...
                # Download to local disk so recordings survive Telnyx URL expiry
                background_tasks.add_task(_download_recording_local, cc_id, url)
            else:
